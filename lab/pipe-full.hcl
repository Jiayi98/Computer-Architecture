...
## Jiayi Zhou (jz22393)
##
## Descrition of comutations of iaddq instruction:
## This is very similar to partB.
## iaddq is to add immediate data to a reister, so I have a constant
## value V for the immediate data and a register rB which is also 
## the destination of the result (valE) of the computation. 
## In Fetch stage, icode and ifun require 1 byte in total. There is a register
## rB so another 1 byte is needed. 8 bytes are needed for instruction constant valC.
## Thus PC increments 10 as result in this stage.
## In Decode stage, read register rB to valB.
## In Execute stage, compute the sum of valC and valB and save it into valE.
## Because of the computation, conditional code could be set.
## In Write Back stage, write the valE into destination rB
## In PC stage, update the program counter.
